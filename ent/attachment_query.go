...
func (aq *AttachmentQuery) QueryComments() *CommentQuery {
...
func (aq *AttachmentQuery) WithComments(opts ...func(*CommentQuery)) *AttachmentQuery {
	query := (&CommentClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AttachmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Attachment, error) {
	...
	if query := aq.withPost; query != nil {
		if err := aq.loadPost(ctx, query, nodes,
			func(n *Attachment) { n.Edges.Post = []*Post{} },
			func(n *Attachment, e *Post) { n.Edges.Post = append(n.Edges.Post, e) }); err != nil {
			return nil, err
		}
	}
	if query := aq.withComments; query != nil {
		if err := aq.loadComments(ctx, query, nodes,
			func(n *Attachment) { n.Edges.Comments = []*Comment{} },
			func(n *Attachment, e *Comment) { n.Edges.Comments = append(n.Edges.Comments, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AttachmentQuery) loadPost(ctx context.Context, query *PostQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *Post)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(attachment.PostTable)
		s.Join(joinT).On(s.C(post.FieldID), joinT.C(attachment.PostPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(attachment.PostPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(attachment.PostPrimaryKey[1]))
...
func (aq *AttachmentQuery) loadComments(ctx context.Context, query *CommentQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *Comment)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Attachment)
	nids := make(map[int]map[*Attachment]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(attachment.CommentsTable)
		s.Join(joinT).On(s.C(comment.FieldID), joinT.C(attachment.CommentsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(attachment.CommentsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(attachment.CommentsPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (as *AttachmentSelect) Modify(modifiers ...func(s *sql.Selector)) *AttachmentSelect {
