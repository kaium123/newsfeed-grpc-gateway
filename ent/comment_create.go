...
func (cc *CommentCreate) AddAttachmentIDs(ids ...int) *CommentCreate {
...
func (cc *CommentCreate) AddReactIDs(ids ...int) *CommentCreate {
	cc.mutation.AddReactIDs(ids...)
	return cc
}

// AddReacts adds the "reacts" edges to the React entity.
func (cc *CommentCreate) AddReacts(r ...*React) *CommentCreate {
	ids := make([]int, len(r))
	for i := range r {
		ids[i] = r[i].ID
	}
	return cc.AddReactIDs(ids...)
}
...
func (cc *CommentCreate) defaults() {
	...
	if _, ok := cc.mutation.ParentCommentID(); !ok {
		v := comment.DefaultParentCommentID
		cc.mutation.SetParentCommentID(v)
	}
}
...
func (cc *CommentCreate) createSpec() (*Comment, *sqlgraph.CreateSpec) {
	...
	if value, ok := cc.mutation.ParentCommentID(); ok {
		_spec.SetField(comment.FieldParentCommentID, field.TypeInt, value)
		_node.ParentCommentID = value
	}
	...
	if nodes := cc.mutation.ReactsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
