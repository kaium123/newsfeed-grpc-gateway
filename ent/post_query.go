...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := pq.withAuthor; query != nil {
		if err := pq.loadAuthor(ctx, query, nodes,
			func(n *Post) { n.Edges.Author = []*User{} },
			func(n *Post, e *User) { n.Edges.Author = append(n.Edges.Author, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PostQuery) loadAttachments(ctx context.Context, query *AttachmentQuery, nodes []*Post, init func(*Post), assign func(*Post, *Attachment)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(post.AttachmentsTable)
		s.Join(joinT).On(s.C(attachment.FieldID), joinT.C(post.AttachmentsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(post.AttachmentsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(post.AttachmentsPrimaryKey[0]))
...
func (pq *PostQuery) loadReacts(ctx context.Context, query *ReactQuery, nodes []*Post, init func(*Post), assign func(*Post, *React)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(post.ReactsTable)
		s.Join(joinT).On(s.C(react.FieldID), joinT.C(post.ReactsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(post.ReactsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(post.ReactsPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (pq *PostQuery) loadAuthor(ctx context.Context, query *UserQuery, nodes []*Post, init func(*Post), assign func(*Post, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(post.AuthorTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(post.AuthorPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(post.AuthorPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(post.AuthorPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
