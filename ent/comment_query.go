...
func (cq *CommentQuery) QueryAttachments() *AttachmentQuery {
	query := (&AttachmentClient{config: cq.config}).Query()
...
func (cq *CommentQuery) QueryReacts() *ReactQuery {
	query := (&ReactClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CommentQuery) Clone() *CommentQuery {
	...
	return &CommentQuery{
		...
		withAttachments: cq.withAttachments.Clone(),
		withPost:        cq.withPost.Clone(),
		withReacts:      cq.withReacts.Clone(),
...
func (cq *CommentQuery) WithAttachments(opts ...func(*AttachmentQuery)) *CommentQuery {
	query := (&AttachmentClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withAttachments = query
	return cq
}
...
func (cq *CommentQuery) WithReacts(opts ...func(*ReactQuery)) *CommentQuery {
	query := (&ReactClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := cq.withAttachments; query != nil {
		if err := cq.loadAttachments(ctx, query, nodes,
			func(n *Comment) { n.Edges.Attachments = []*Attachment{} },
			func(n *Comment, e *Attachment) { n.Edges.Attachments = append(n.Edges.Attachments, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := cq.withReacts; query != nil {
		if err := cq.loadReacts(ctx, query, nodes,
			func(n *Comment) { n.Edges.Reacts = []*React{} },
			func(n *Comment, e *React) { n.Edges.Reacts = append(n.Edges.Reacts, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CommentQuery) loadAttachments(ctx context.Context, query *AttachmentQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Attachment)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(comment.AttachmentsTable)
		s.Join(joinT).On(s.C(attachment.FieldID), joinT.C(comment.AttachmentsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(comment.AttachmentsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(comment.AttachmentsPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*Attachment](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
...
func (cq *CommentQuery) loadReacts(ctx context.Context, query *ReactQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *React)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Comment)
	nids := make(map[int]map[*Comment]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(comment.ReactsTable)
		s.Join(joinT).On(s.C(react.FieldID), joinT.C(comment.ReactsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(comment.ReactsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(comment.ReactsPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	neighbors, err := withInterceptors[[]*React](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "reacts" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
