...
func (uq *UserQuery) QueryAttachments() *AttachmentQuery {
	query := (&AttachmentClient{config: uq.config}).Query()
...
func (uq *UserQuery) QueryReactedUser() *ReactQuery {
	query := (&ReactClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withReactedUser: uq.withReactedUser.Clone(),
...
func (uq *UserQuery) WithReactedUser(opts ...func(*ReactQuery)) *UserQuery {
	query := (&ReactClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withReactedUser = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withReactedUser; query != nil {
		if err := uq.loadReactedUser(ctx, query, nodes,
			func(n *User) { n.Edges.ReactedUser = []*React{} },
			func(n *User, e *React) { n.Edges.ReactedUser = append(n.Edges.ReactedUser, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadReactedUser(ctx context.Context, query *ReactQuery, nodes []*User, init func(*User), assign func(*User, *React)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.React(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ReactedUserColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_reacted_user
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_reacted_user" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_reacted_user" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) loadPosts(ctx context.Context, query *PostQuery, nodes []*User, init func(*User), assign func(*User, *Post)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.PostsTable)
		s.Join(joinT).On(s.C(post.FieldID), joinT.C(user.PostsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.PostsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.PostsPrimaryKey[0]))
