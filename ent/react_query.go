...
func (rq *ReactQuery) QueryPosts() *PostQuery {
...
func (rq *ReactQuery) QueryComments() *CommentQuery {
...
// QueryReactedUser chains the current query on the "reacted_user" edge.
func (rq *ReactQuery) QueryReactedUser() *CommentQuery {
	query := (&CommentClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (rq *ReactQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (rq *ReactQuery) FirstIDX(ctx context.Context) int {
...
func (rq *ReactQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (rq *ReactQuery) OnlyIDX(ctx context.Context) int {
...
func (rq *ReactQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (rq *ReactQuery) IDsX(ctx context.Context) []int {
	ids, err := rq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (rq *ReactQuery) Clone() *ReactQuery {
	...
	return &ReactQuery{
		...
		withPosts:       rq.withPosts.Clone(),
		withComments:    rq.withComments.Clone(),
		withReactedUser: rq.withReactedUser.Clone(),
...
func (rq *ReactQuery) WithPosts(opts ...func(*PostQuery)) *ReactQuery {
	query := (&PostClient{config: rq.config}).Query()
...
func (rq *ReactQuery) WithComments(opts ...func(*CommentQuery)) *ReactQuery {
	query := (&CommentClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithReactedUser tells the query-builder to eager-load the nodes that are connected to
// the "reacted_user" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *ReactQuery) WithReactedUser(opts ...func(*CommentQuery)) *ReactQuery {
	query := (&CommentClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withReactedUser = query
	return rq
}
...
func (rq *ReactQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*React, error) {
	...
	if query := rq.withPosts; query != nil {
		if err := rq.loadPosts(ctx, query, nodes,
			func(n *React) { n.Edges.Posts = []*Post{} },
			func(n *React, e *Post) { n.Edges.Posts = append(n.Edges.Posts, e) }); err != nil {
			return nil, err
		}
	}
	if query := rq.withComments; query != nil {
		if err := rq.loadComments(ctx, query, nodes,
			func(n *React) { n.Edges.Comments = []*Comment{} },
			func(n *React, e *Comment) { n.Edges.Comments = append(n.Edges.Comments, e) }); err != nil {
			return nil, err
		}
	}
	if query := rq.withReactedUser; query != nil {
		if err := rq.loadReactedUser(ctx, query, nodes,
			func(n *React) { n.Edges.ReactedUser = []*Comment{} },
			func(n *React, e *Comment) { n.Edges.ReactedUser = append(n.Edges.ReactedUser, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ReactQuery) loadPosts(ctx context.Context, query *PostQuery, nodes []*React, init func(*React), assign func(*React, *Post)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*React)
	nids := make(map[int]map[*React]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(react.PostsTable)
		s.Join(joinT).On(s.C(post.FieldID), joinT.C(react.PostsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(react.PostsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(react.PostsPrimaryKey[1]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*React]struct{}{byID[outValue]: {}}
...
func (rq *ReactQuery) loadComments(ctx context.Context, query *CommentQuery, nodes []*React, init func(*React), assign func(*React, *Comment)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*React)
	nids := make(map[int]map[*React]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(react.CommentsTable)
		s.Join(joinT).On(s.C(comment.FieldID), joinT.C(react.CommentsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(react.CommentsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(react.CommentsPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (rq *ReactQuery) loadReactedUser(ctx context.Context, query *CommentQuery, nodes []*React, init func(*React), assign func(*React, *Comment)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*React)
	nids := make(map[int]map[*React]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(react.ReactedUserTable)
		s.Join(joinT).On(s.C(comment.FieldID), joinT.C(react.ReactedUserPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(react.ReactedUserPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(react.ReactedUserPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "reacted_user" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (rq *ReactQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(react.Table, react.Columns, sqlgraph.NewFieldSpec(react.FieldID, field.TypeInt))
	_spec.From = rq.sql
